...
export async function authorizeScim(
  ...
  const row = await dbAdmin.query.scimTokens.findFirst({
    where: eq(schema.scimTokens.tokenHash, hashSecret(token)),
  });
...
async function membershipForUser(orgId: string, userId: string) {
  return dbAdmin.query.memberships.findFirst({
    where: and(
...
export async function userResourceEtag(
  orgId: string,
...
export async function userResource(
  ...
  return {
    ...
    groups: membership?.active
      ? [{ value: membership.id, display: membership.role }]
      : [],
    meta: {
      resourceType: "User",
      created: user.createdAt.toISOString(),
      lastModified: user.createdAt.toISOString(),
      location: resourceLocation(request, "Users", user.id),
    },
  };
}
...
export async function listScimUsers(
  ...
  const orgScope = scopedUserIds.length
    ? or(
        ...
        inArray(schema.users.id, scopedUserIds),
      )
    : eq(schema.users.scimOrgId, orgId);
  ...
  return {
    ...
    totalResults: Number(totalRow[0]?.value ?? 0),
    startIndex,
    itemsPerPage: rows.length,
    Resources: await Promise.all(
      rows.map((user) => userResource(request, orgId, user)),
    ),
  };
}
...
export async function resolveMappedRole(
  orgId: string,
  groupIds: string[] | undefined,
): Promise<Role | null> {
  if (!groupIds || groupIds.length === 0) return null;
  const groups = await dbAdmin.query.directoryGroups.findMany({
    where: and(
      eq(schema.directoryGroups.orgId, orgId),
      or(...groupIds.map((id) => eq(schema.directoryGroups.externalId, id))),
    ),
  });
  const roles = Array.from(
    new Set(
      groups
        .map((group) => group.mappedRole)
...
export async function createScimUser(
  ...
  const email =
    typeof input.userName === "string" ? input.userName.toLowerCase() : "";
  ...
  const displayName =
    typeof input.displayName === "string" && input.displayName.trim()
      ...
      : email;
  const externalId =
    typeof input.externalId === "string" ? input.externalId : null;
  const active = input.active !== false;
  const groups = Array.isArray(input.groups)
    ? input.groups.filter((value): value is Record<string, unknown> =>
        Boolean(value && typeof value === "object"),
      )
    : [];
  const groupIds = groups
    .map((group) => (typeof group.value === "string" ? group.value : ""))
    .filter(Boolean);
  ...
  if (role)
    await dbAdmin
      ...
      .values({ userId: user.id, orgId, role, active, sessionVersion: 1 });
  await auditScim(orgId, "scim.user_provisioned", "user", user.id, {
    mappedRole: role,
    active,
  });
  return user;
...
export async function updateScimUser(
  ...
  if (Object.prototype.hasOwnProperty.call(input, "externalId")) {
    if (input.externalId === null) externalId = null;
    else if (typeof input.externalId === "string" && input.externalId.trim()) {
      externalId = input.externalId.trim();
    } else {
      throw new Error("externalId must be a non-empty string or null");
    }
  } else if (replace) {
    externalId = null;
  }
  ...
  if (membership && active !== undefined && membership.active !== active) {
    await dbAdmin
      ...
      .set({ active, sessionVersion: membership.sessionVersion + 1 })
      .where(eq(schema.memberships.id, membership.id));
  }
  const updated =
    (await dbAdmin.query.users.findFirst({ where: eq(schema.users.id, id) })) ??
    user;
  await auditScim(orgId, "scim.user_updated", "user", id, {
    active: active ?? membership?.active ?? null,
  });
  return updated;
}
...
export async function deleteScimUser(
  ...
  await dbAdmin
    .update(schema.memberships)
    .set({ active: false, sessionVersion: membership.sessionVersion + 1 })
    .where(eq(schema.memberships.id, membership.id));
  await auditScim(orgId, "scim.user_deprovisioned", "user", id, {
    active: false,
  });
  return true;
}

export async function groupResource(
  request: Request,
  orgId: string,
  group: typeof schema.directoryGroups.$inferSelect,
) {
  const members = group.mappedRole
    ? await dbAdmin.query.memberships.findMany({
        where: and(
          eq(schema.memberships.orgId, orgId),
          eq(schema.memberships.role, group.mappedRole),
        ),
      })
    : [];
  return {
    ...
    meta: {
      resourceType: "Group",
      location: resourceLocation(request, "Groups", group.externalId),
    },
  };
}
...
export async function listScimGroups(request: Request, orgId: string) {
  const groups = await dbAdmin.query.directoryGroups.findMany({
    where: eq(schema.directoryGroups.orgId, orgId),
  ...
  return {
    ...
    Resources: await Promise.all(
      groups.map((group) => groupResource(request, orgId, group)),
    ),
  };
}

export async function upsertScimGroup(
  orgId: string,
  input: Record<string, unknown>,
) {
  const displayName =
  ...
  const existing = await dbAdmin.query.directoryGroups.findFirst({
    where: and(
      eq(schema.directoryGroups.orgId, orgId),
      eq(schema.directoryGroups.externalId, externalId),
    ),
  });
  if (existing) throw new Error("A group with this id already exists");
  const inserted = await dbAdmin
    .insert(schema.directoryGroups)
    .values({ orgId, externalId, displayName, mappedRole })
    .returning();
  await auditScim(orgId, "scim.group_created", "directory_group", externalId, {
    displayName,
    mappedRole,
  });
  return inserted[0];
...
export async function patchScimGroup(
  ...
  const group = await dbAdmin.query.directoryGroups.findFirst({
    where: and(
      eq(schema.directoryGroups.orgId, orgId),
      eq(schema.directoryGroups.externalId, externalId),
    ),
  ...
  if (group.mappedRole) {
    ...
    for (const userId of additions) {
      ...
      if (membership && membership.role !== group.mappedRole)
  ...
  for (const raw of operations) {
    ...
    for (const member of members) {
      ...
      if (operation.op?.toLowerCase() === "remove") {
        ...
        await dbAdmin
          .update(schema.memberships)
          .set({ active: true, sessionVersion: membership.sessionVersion + 1 })
          .where(eq(schema.memberships.id, membership.id));
      }
    }
  }
