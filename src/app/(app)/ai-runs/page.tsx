    ...
    return (
      <div className="scroll-mt-28" data-tour-target={TOUR_TARGETS.aiEvidenceList}>
        ...
        <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <div className="card p-4"><p className="text-2xl font-semibold text-slate-900">{runs.length}</p><p className="mt-1 text-sm text-gray-500">Inspectable runs</p><p className="mt-0.5 text-xs text-gray-500">tenant-scoped traces</p></div>
          <div className="card p-4"><p className="text-2xl font-semibold text-emerald-700">{passingRuns}</p><p className="mt-1 text-sm text-gray-500">Hard-gate passes</p><p className="mt-0.5 text-xs text-gray-500">of {checkedRuns} evaluated runs</p></div>
          <div className="card p-4"><p className="text-2xl font-semibold text-amber-700">{repairRuns}</p><p className="mt-1 text-sm text-gray-500">Repair rescues</p><p className="mt-0.5 text-xs text-gray-500">invalid first pass → repair</p></div>
          <div className="card p-4"><p className="text-2xl font-semibold text-indigo-700">{runs.filter((run) => run.artifactType === "plan").length}</p><p className="mt-1 text-sm text-gray-500">Plan artifacts</p><p className="mt-0.5 text-xs text-gray-500">approval-linked where available</p></div>
        </div>
        <div className="card overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="border-b border-gray-100 text-left text-[11px] uppercase tracking-wide text-gray-500"><tr><th className="px-5 py-3">Run</th><th className="px-5 py-3">Artifact</th><th className="px-5 py-3">Provider / prompt</th><th className="px-5 py-3 text-right">Evidence</th><th className="px-5 py-3 text-right">Latency</th><th className="px-5 py-3 text-right">Cost</th><th className="px-5 py-3">Outcome</th></tr></thead>
              <tbody className="divide-y divide-gray-100">
                {runs.map((run) => {
                  const summary = summaries.get(run.id);
                  return <tr key={run.id} className="hover:bg-slate-50"><td className="px-5 py-3"><Link href={`/ai-runs/${run.id}`} className="font-mono text-xs text-indigo-700 hover:underline">{run.id.slice(0, 8)}</Link><p className="mt-0.5 text-xs text-gray-500">{run.createdAt.toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</p></td><td className="px-5 py-3 capitalize text-gray-700">{run.artifactType.replace("_", " ")}</td><td className="px-5 py-3"><p className="text-gray-700">{run.provider}</p><p className="font-mono text-[11px] text-gray-500">{run.promptVersion ?? "—"}</p></td><td className="px-5 py-3 text-right">{summary?.total ? <span className={`badge ${summary.hardGatePassed ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{summary.passed}/{summary.total}</span> : <span className="text-xs text-gray-500">Legacy</span>}</td><td className="px-5 py-3 text-right tabular-nums text-gray-700">{run.latencyMs == null ? "—" : `${(run.latencyMs / 1000).toFixed(1)}s`}</td><td className="px-5 py-3 text-right tabular-nums text-gray-700">{run.costUsd == null ? (run.dataOrigin === "fixture" ? <span title="Synthetic fixture; provider spend was not measured">Not priced</span> : "—") : `$${Number(run.costUsd).toFixed(4)}`}</td><td className="px-5 py-3"><span className={`badge ${outcomeTone[run.finalOutcome ?? run.status] ?? "bg-gray-100 text-gray-600"}`}>{run.finalOutcome ?? run.status}</span></td></tr>;
                })}
                {runs.length === 0 && <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-500">No matching evidence yet. Generate a plan to create the first evidence packet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
